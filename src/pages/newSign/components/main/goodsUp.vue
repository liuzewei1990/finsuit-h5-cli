  ...
  <div class="main-goods-up" v-show="goods.length">
    ...
    <div class="box-scroll">
      <ul class="box-list" :style="{width:`${boxWidth}px`}">
        <!-- <ul class="box-list"> -->
        <!-- item -->
        <li class="item dev" v-for="(item,index) in goods" :key="index" ref="item" @click="getGoodsRedirectUrl(item)" v-track:ACB0F007.click="()=>{}">
    ...
    async getGoodsList () {
      try {
        ...
        if (this.$utils.isArray(data)) {
          console.log("this.$api.getGoodsList----------", data);
    ...
    async getGoodsRedirectUrl (item) {
      try {
        ...
        if (data.is_enabled == "0") {
          // H5跳转
          if (data.redirect == 1) {
            if (this.APP_FLAG === "PC" || this.APP_FLAG === "BC") {
          ...
          if (data.redirect == 2) {
            ...
            let http = this.APP_FLAG == "BC" ? "besharp" : "pincai";
