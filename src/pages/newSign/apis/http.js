    ...
    postPoint: function (baseURL, url, params = {}) {
        ...
        param_key["param"] = {
            FUNCTION_LOG_LIST: [{
                ...
                APP_PLACE: 102,                                                                                  // 日志位置
                FROM_TYPE: 15,                                                                                   // 日志分类
                NETWORK_TYPE: sessionStorage.getItem("NETWORK_TYPE") || 2,                                       // 网络类型 1:数据流量  2: WIFI
                SOURCE_ID: 1,                                                                                    // 渠道来源标识 1:A版本 2:B版本
                CREATE_TIME: Date.now(),                                                                         // 创建时间戳，精确到毫秒的时间戳
                SOURCE_URL: encodeURIComponent(window.location.href),                                            // 链接地址URL 对于H5活动，填当前页URL
                APP_MARKET_CODE: "h5",                                                                           // 安卓
                ID: sessionStorage.getItem("ID") || "",                                                          // 会员ID 如果已经登录需要传
                MEMBER_ID: sessionStorage.getItem("ID") || "",                                                   // 会员ID 如果已经登录需要传
                FROM_PR1: sessionStorage.getItem("APP_FLAG"),                                                    // Appid
