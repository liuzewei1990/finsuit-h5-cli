...
export default {
    ...
    openWebviewReturn (url = "", session, callBack) {
        const CACHE_KEY = "PAGE_APPEAR_KEY";

        // webview 返回时会触发
        window.pageAppear = function () {
            let cacheSession = sessionStorage.getItem(CACHE_KEY);
        ...
        // 记录这个值
        sessionStorage.setItem(CACHE_KEY, session);

        this.openWebview(url, title, NEED_LOGIN);
    },
    /**
     * 分享文本+链接
     * @param {Obj} data 
